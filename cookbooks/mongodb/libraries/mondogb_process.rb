class Chef
  class Recipe
      ...
      when "sysv"
        ...
        subscribes :restart, resources(:template => "/etc/init.d/#{service_name}") if node[:mongodb][:system_init] == 'sysv'
        subscribes :restart, resources(:template => "/etc/init/#{service_name}.conf") if node[:mongodb][:system_init] == 'upstart'
			end
